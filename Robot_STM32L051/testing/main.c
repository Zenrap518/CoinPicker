...
void SendATCommand(char* s)
{
	...
	waitms(10);
	eputs2(s);
	// Use timeout version of egets2 to ensure we don't block indefinitely
	egets2_timeout(buff, sizeof(buff) - 1, 100); // 100ms timeout
	...
	while (1) // Loop indefinitely
	{
		...
		if (ReceivedBytes2() > 0) // Something has arrived
		{
			...
			if (c == '!') // Master is sending message
			{
				...
				// Read with timeout to ensure we get a complete message
				int result = egets2_timeout(buff, sizeof(buff) - 1, 50); // 50ms timeout
				...
				{
					printf("Timeout waiting for complete message\r");
				}
			}
