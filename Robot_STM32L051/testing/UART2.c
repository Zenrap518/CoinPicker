...
{
	// Non-blocking version: read available bytes from the comms port until end of string,
	// newline is encountered, or no more data is available.
	// Buffer is terminated with null
	// returns number of characters read on success
	// returns 0 if no complete message yet
	...
	if (GetBufCount2(&ComRXBuffer2) == 0) return 0;
	
	while ((Len < Max-1) && (GetBufCount2(&ComRXBuffer2) > 0))
	{
		c = GetBuf2(&ComRXBuffer2);
		s[Len++] = c;
		
		if (c == LINEFEED) // Found end of message
			break;
	}
	...
	if (Len > 0)
	{
		...
		// Only return a complete message if we found a linefeed or filled the buffer
		if (c == LINEFEED || Len >= (Max-1))
			return Len;
		else
		{
...
// Add a new function to read complete messages with timeout
int egets2_timeout(char *s, int Max, int timeout_ms)
{
	int Len = 0;
	int start_time = 0;
	int current_time = 0;
	...
	while (current_time < timeout_ms)
	{
		Len = egets2(s, Max);
		if (Len > 0) 
			return Len; // Complete message received
